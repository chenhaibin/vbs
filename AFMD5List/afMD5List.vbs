...
sub EnumFolder (strFolder)
	...
	Dim oFolder, owshshell, strMD5
	
	Dim item, strCommand, strPath
	...
	for each item in ofolder.SubFolders
	       ...
	       'subDebugMsg item.path
     	next

	for each item in ofolder.files
		strPath = chr(34) & replace (item.path, "\", "/") & chr(34)
		strCommand = "md5.cmd " & strPath & "," & item.datelastmodified
		oWSHShell.run strCommand, 0, true 
		'subDebugMsg item.Path & "," & item.datelastmodified
	next
