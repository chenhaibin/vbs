' ***********************************************************
' Name    : HxBackup.vbs - Backs up files to a location 
' Author  : Adrian Farnell
' Date    : 09.12.2002
' Version : 1.0
' Notes   : Uses a backup.ini where you can specify wildcards etc..
'
...
if oFSO.fileExists ("backup.ini") = true then
	subDebugMsg "Found backup.ini"
else
	subErrMsg "Backup.ini missing"
end if
...
sub subCreateBackupDir (strBackupFolder)

	subDebugMsg "Backup to: " & strBackupFolder
	
end Sub

sub subCopyFolder (strCopyFolder, strCopy_Move)

	Select case strCopy_Move

		case "copy"
			subDebugMsg "Copying folder: " & strCopyFolder
		case "move"
			subDebugMsg "Moving folder: " & strCopyfolder
		case else
...
sub subCopyWildCardFiles (strWildCard, strCopy_Move)
	
	Select case strCopy_Move

		case "copy"
			subDebugMsg "Copying files: " & strCopyFolder
		case "move"
			subDebugMsg "Moving files: " & strCopyfolder
		case else
...
sub subCopySingleFile
	
	Select case strCopy_Move

		case "copy"
			subDebugMsg "Copying file: " & strCopyFolder
		case "move"
			subDebugMsg "Moving file: " & strCopyfolder
		case else
...
sub subErrMsg (strMsg)
	...
	if err.number <> 0 then 
		oWshShell.Run "logevent -s E " & chr(34) & strMsg & chr(34)
	end if
