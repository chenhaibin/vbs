...
'sub to exercise all functions
'subtestrun

Dim ofso, ofile, oNewFile, oWshNetwork

Dim sFileName, sNewFile, strLine 

sFileName = "NDMv1_0responsefile.txt"
sNewFile = "c:\newfile.txt"
...
if ofso.fileexists (sFileName) = true then 
	...
	if err.number <> 0 then 
		...
		subErrMsg "Could not open file: " & sFileName
	
	end if
else 

		subErrMsg "Could not find file: " & sFileName

end if

if ofso.fileexists (sNewFile) = true then
	
	subErrMsg "File exists."
	
else
	
	set oNewfile = ofso.opentextfile (sNewFile, 2, true)
...
Do while ofile.atendofstream <> true
	...
	if InStr (strLine, "VBnodenameManagerVB") <> 0 then
		strLine = Replace (strLine, "VBnodenameManagerVB", oWshNetwork.ComputerName)
		oNewFile.WriteLine strLine
	elseif Instr (strLine, "VBnodenameNetworkVB") <> 0 then
		strLine = Replace (strLine, "VBnodenameNetworkVB", "WASCELL1")
		oNewFile.WriteLine strLine
	elseif Instr (strLine, "VBhostNameOrIPAddressVB") <> 0 then 
		'cell managers name in here
		strLine = Replace (strLine, "VBhostNameOrIPAddressVB", oWshNetwork.ComputerName)
		oNewFile.WriteLine strLine
	else
		oNewFile.WriteLine strLine
	end if
