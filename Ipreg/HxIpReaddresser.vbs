...
objwshshell.regread ("HKEY_LOCAL_MACHINE\System\ControlSet001\Services\e100b\type")


If Err.Number <> 0 Then

		Wscript.Echo vbcrlf & " E100B Key does not exist, this server does not"
		wscript.echo " contain an Intel EtherExpress Pro 100+ NIC"
		wscript.quit
		'Wscript.Echo " Err #" & Err.Number & ", " & Err.Description
	Else
		wscript.echo vbcrlf & " server contains at least one Intel EtherExpress Pro" & vbcrlf	
		
		objwshshell.regread ("HKEY_LOCAL_MACHINE\System\ControlSet001\Services\e100b1\type")

		If Err.Number <> 0 Then 
			wscript.Echo " server does not have a card labelled e100b1." & vbcrlf & " Script ending"
			If strNIC="e100b1" then wscript.quit		
		Else
			wscript.Echo " server has a NIC labelled in registry as e100b1"
		End If
		
		
		objwshshell.regread ("HKEY_LOCAL_MACHINE\System\ControlSet001\Services\e100b2\type")

		If Err.Number <> 0 Then 

			wscript.Echo " server does not have a card labelled e100b2"
			If strNIC="e100b2" then wscript.quit				
		Else
			wscript.Echo " server has a NIC labelled in registry as e100b2"
		End If

		objwshshell.regread ("HKEY_LOCAL_MACHINE\System\ControlSet001\Services\e100b3\type")

		If Err.Number <> 0 Then 
			wscript.Echo " server does not have a card labelled e100b3." & vbcrlf & " Script ending"
			If strNIC="e100b3" then wscript.quit
		Else
			wscript.Echo " server has a NIC labelled in registry as e100b3"
		End If
