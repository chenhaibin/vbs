' ***********************************************************
' Name    : AfCreProcess.vbs
' Author  : Adrian Farnell
' Date    : 14.11.2002
' Version : 1.0
' Notes   : Logs each process created
'
...
'Dim strings
Dim strComputer, strLogDir

'Dim objects
Dim objWMIService, owshnetwork, objLatestProcess
...
strLogDir = "c:\temp\logs\CreateProcess"
...
Do While i = 0
    ...
    sublogtofile Now & " " & owshnetwork.computername & " "& objLatestProcess.TargetInstance.Name & " (PID: " & objLatestProcess.TargetInstance.ProcessID &", "& objLatestProcess.TargetInstance.ExecutablePath& ")" & vbcrlf, strLogDir '
    'subDebugMsg Now.objLatestProcess.CommandLine
    subDebugMsg Now & " " & owshnetwork.computername & " "& objLatestProcess.TargetInstance.Name & " (PID: " & objLatestProcess.TargetInstance.ProcessID &", "& objLatestProcess.TargetInstance.ExecutablePath& ")" & vbcrlf
...
sub subTestRun
	...
	call subLogtoFile ("Hello" & vbcrlf, "c:\temp\logs\test")
...
sub subLogtoFile (strMsg, strLogDir)
	...
	if ofso.fileexists (strLogDir & "\CreProcess_" & oWshNetwork.Computername &"_"& strYYYY & strMM & strDD & ".log") = true then
		subDebugMsg "file does exist"
		
		'open file for appending
		set ofile = ofso.opentextfile (strLogDir & "\CreProcess_" & oWshNetwork.Computername &"_"& strYYYY & strMM & strDD & ".log",8)
	else
		subDebugMsg "file doesn't exist"
		
		'create file
		set ofile = ofso.createtextfile (strLogDir & "\CreProcess_" & oWshNetwork.Computername &"_"& strYYYY & strMM & strDD & ".log")
	end if
