...
'
'  subLogToFile ( strMsg, strLogDir ) - Logs Message into file in directory specified, filename will be based on date, server name and value in strPrefix.
...
strStartDate = DateAdd ("D", -1, Date)
strFinishDate = Date

set objWMIService = GetObject ("winmgmts:{impersonationLevel=impersonate}!\\" & strComputer & "\root\cimv2")

set colEvents = objWMIService.execQuery ("select * from Win32_NTLogEvent where TimeWritten >= '" & strStartDate & "' and TimeWritten < '" & strFinishDate & "'")
...
For each objEvent in colEvents
	...
	strLine = strDD & "/" & strMM & "/" & strYYYY & " " & strHH & ":" & strMin & ":" & strSS
	strLine = strLine &"#" & objEvent.Type & "#" & objEvent.SourceName & "#" & objEvent.Category
	strLine = strLine &"#" & objEvent.EventCode & "#" & objEvent.User & "#" & objEvent.ComputerName
	strLine = strLine &"#" & objEvent.Logfile & "#" & strMessage & vbcrlf

	subLogtoFile strLine, strLogDir 
	subdebugmsg strLine
...
sub subCreateDir (strDir)
	...
	'what is the day
	strDD = Left ( DateAdd ("D", -1, strDate), 2)
