...
if BoolUninstall = FALSE then 

	subChangeSmartHost "10.190.32.46"

else
	subDebugMsg "Uninstalling...."
	
	subChangeSmartHost oShell.RegRead ("HKLM\Software\Halifax\HxChangeSMTP")
	
	oShell.RegDelete ("HKLM\Software\Halifax\HxChangeSMTP")
	
end if

subdebugmsg "Stopping SMTP Service"

oshell.run "net stop smtpsvc" , 0, true

subdebugmsg "Starting SMTP Service"

oshell.run "net start smtpsvc", 0, true

set oShell = nothing
	
' ------------------------------------------( cut ) ----------------------------------------------

sub subChangeSmarthost (strSmartHost)

	Dim oADSMTP, oWshShell

	'On error resume next

	err.clear

	set oADSMTP = getobject ("IIS://localhost/smtpsvc/1")
	
	set oWshShell = createobject ("wscript.shell")
	
	if err.number <> 0 then 
		subErrMsg "SMTP Object not available"
		wscript.quit (1)
	end if

	On error goto 0

	submsg "SMTP Service configured to talk to: " & oADSMTP.smarthost
	
	'stick in registry here
	oWshShell.regWrite "HKLM\Software\Halifax\HxChangeSMTP", oADSMTP.smartHost
	
	'check it got there
	if oWshShell.regRead ("HKLM\Software\Halifax\HxChangeSMTP") = oADSMTP.smartHost then
		subDebugMsg "SMTP backout entry set to " & oWshShell.regRead ("HKLM\Software\Halifax\HxChangeSMTP")
	else
		subErrMsg "Backout entry could not be written to registry. No changes have been made"
	end if
	
	submsg "Setting SMTP Smarthost to: " & strSmartHost

	on error resume next

	err.clear

	oADSMTP.Put "SmartHost", strSmartHost

	oADSMTP.SetInfo

	if err.number <> 0 then 
		subErrMsg "SMTP has not been changed, smarthost is : " & oADSMTP.smarthost
	end if

	on error goto 0 

	if oADSMTP.smarthost = strSmartHost then
		subMsg "succesfully changed SMTP to " & strSmartHost
	else
		subErrMsg "SMTP has not been changed, smarthost is : " & oADSMTP.smarthost
		wscript.quit (1)
	end if
