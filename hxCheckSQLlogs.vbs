' ***********************************************************
' Name    : HxCheckSQLLogs.vbs
' Author  : Adrian Farnell
' Date    : 27.01.2003
' Version : 1.0
' Notes   : Checks SQL Logs to see if change succeeds
'
...
'Dim strings
Dim strLogLocation, strline, strSSNETLIB

set oFso = createobject ("scripting.filesystemobject")

if ofso.fileexists ("E:\Data\MSSQL\LOG") = true then
	subDebugMsg "Found Errorlog file on e:"
	
	strLogLocation = "E:\Data\MSSQL\LOG"
else 
	
	subErrMsg "Cannot find ErrorLog file"
	
end if

set oFile = ofso.opentextfile (strLogLocation)

Do while ofile.atendofstream = false

	strLine = ofile.readline
	
	if instr (strLine, "Using 'SSNETLIB.DLL") <> 0 then
		subDebugMsg "Found ssnetlib line"
		strSSNETLIB = strLine
	else
		'subDebugMsg "Not found ssnetlib"
		
	end if 

Loop

subDebugMsg strSSNETLIB

if strSSNETLIB = "" then
	subErrMsg "Could not determine version of ssNETLIB.dll"
end if

strSSNETLIB = Mid (strSSNETLIB, instr (strSSNETLIB, "8.0"))

strSSNETLIB = replace (strSSNETLIB, "8.0" , "")
strSSNETLIB = replace (strSSNETLIB, "'" , "")
strSSNETLIB = replace (strSSNETLIB, "." , "")

subDebugMsg strSSNETLIB

Select case strSSNETLIB

	case 760
		subdebugMsg "SQL Server 2000 now at SP3"	
	case 534 
		subErrMsg "SQL Server 2000 failed to be updated"	
	case 636
		subMsg "SQL Server 2000 has been updated successfully"	
	case else
		subErrMsg "Could not determine ssnetlib.dll version"	
end select 
...
sub subMsg (strMsg)
	...
	oWshShell.logevent 4,Now & " " & wscript.scriptname & " Err: " & strMsg
