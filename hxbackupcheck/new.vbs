...
streveryhourtext = Mid (streveryhourtext,64)
streveryhourtext = replace (streveryhourtext, "HHmm", HH & "00")
streveryhourtext = replace (streveryhourtext, "DD", DD)
streveryhourtext = replace (streveryhourtext, "MM", MM)
streveryhourtext = replace (streveryhourtext, "YYYY", YYYY)

'generate list of files to check for every hour
 
arreveryhourfiles = split(streveryhourtext, ",")

for each memserver in arrmemservers
	'For each audserver in arraudservers
		for each a in arreveryhourfiles 
			a = replace (a, "<mem servers>", memserver)
